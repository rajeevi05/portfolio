...
interface InkRevealProps {
  ...
  /** Automatically disperse the ink without user input. */
  autoDisperse?: boolean;
  /** Duration of the auto-disperse animation in ms */
  autoDuration?: number;
  /** Called when auto-disperse completes */
  onComplete?: () => void;
}
...
export default function InkReveal({
  ...
  // Auto-disperse: trace a space-filling lissajous path to uncover the layer.
  useEffect(() => {
    if (!autoDisperse) return;
    let raf = 0;
    ...
    const tick = (now: number) => {
      ...
      const t = Math.min(1, (now - start) / autoDuration);
      // Multiple parallel sweepers for organic coverage
      const sweepers = 3;
      for (let s = 0; s < sweepers; s++) {
        const phase = (s / sweepers) * Math.PI * 2;
        const a = t * Math.PI * 6 + phase;
        const b = t * Math.PI * 4 + phase * 1.7;
        const x = w * (0.5 + 0.42 * Math.sin(a));
        const y = h * (0.5 + 0.42 * Math.sin(b));
        addStamp(x, y);
      }
      startLoop();
      if (t < 1) {
        ...
        // Final flood to fully dissolve the mask
        const { w: ww, h: hh } = dimsRef.current;
        for (let i = 0; i < 60; i++) {
          addStamp(Math.random() * ww, Math.random() * hh);
        }
        startLoop();
  ...
  }, [autoDisperse, autoDuration, addStamp, startLoop, onComplete]);
